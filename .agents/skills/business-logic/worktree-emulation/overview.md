...
- **子目录会话的「自己的树」是根，不是 cwd**：面板三处回答「这是不是会话自己的树」（源选择器当前行与 `●`、切源时「选自己的树则清除覆盖而非钉住」、绑定变化时丢掉这种钉住让视图跟着 agent）曾都比原始 cwd，`A/B` 在 worktree 列表里谁也不是——没有行亮、点主树行变成钉住、`worktree_enter` 后抽屉留在原地。`worktreeStatus` 把它本就解析了的调用方根作为 `repoRoot` 返回（仓库外 `null`），客户端 `sessionTree(cwd, repoRoot)`（worktree-view.ts）在 cwd 比根深时用根、cwd 就是根时保留 cwd 拼写（挂载 stats 抓取以此字符串做 key）；面板 `sessionRoot` → `sessionPath = binding?.worktreePath ?? sessionRoot`，三处比较走 `samePath`。旧宿主无 `repoRoot` 时回落原行为（README §6.27；守卫在 `tests/switcher-gate.test.ts`）
- **切换源不闪 `(no branch)`**：`branchOfWorktree(path, worktrees)` 用手头列表立即播种分支——随后的 stats 抓取要跑 `git status` + numstat（大仓库以秒计），占位符绝不能用 `EMPTY_STATS`（branch 为空）。实测 50ms 采样下 header 单次跳变；同胞 `rootOfWorktree(path, worktrees)` 同时预填 `repoRoot`（列表里的 worktree 就是自己的根），切换器不用等 stats 抓完才弹入；子目录会话不在列表里、答 undefined、等 git
...
- **工具 output schema 必须与 RPC 返回键同步**：`STATUS_SCHEMA` 是 `additionalProperties: false`，dsh 按它校验工具输出，未声明的键 = 每次调用 `INVALID_TOOL_OUTPUT`（0.1.19 起 `worktree_status` 因 `remoteBranches`/`mainWorktreePath` 等未声明而一直失败，抽屉直读 RPC 未察觉）；可空字符串写 `oneOf: [null, string]`（README §6.11）。守卫 `tests/worktree-status-schema.test.ts`（剥注释钉 schema 属性表 ≡ `worktreeStatus` 返回类型键；本机有 `@deepseek-ai/dsh-tools` 时再跑真校验器）
- 会话 cwd 不可变（dsh 本体约束）：enter 返回 hint 教模型相对路径用法（file 工具加前缀、shell 传 per-call workdir）。**前缀必须相对会话 cwd 真正解析**——`worktreeRel(cwd, worktreePath)`（worktree.ts，`node:path.relative`、统一正斜杠、相等答 `.`）：worktree 建在仓库根下，会话开在子目录 `<root>/server` 时前缀是 `../.agents/worktrees/<name>`，写死 `.agents/worktrees/<name>` 会让文件工具把文件建进主树（未跟踪、无报错）；hint 与 `bindingNotice`（第 4 参 `rel`，提示回调传 `session.header?.cwd ?? binding.repoRoot`）两处都用它，工具描述也改为「用 hint 里的前缀」（README §6.27；守卫 `tests/worktree-rel-wiring.test.ts` 剥注释扫描两处调用点、逐点变异）
...
## Code Location
`bindingsPath/parseBindings/saveBindings`、`sanitizeName`/`resolveEnterBranch`/`worktreeDir`/`worktreeRel`/`bindingNotice`/`parseWorktreeList`（worktree.ts）、显示推导纯函数 `samePath/viewedPath/sessionTree/branchOfWorktree/rootOfWorktree/badgeRepeatsBranch/splitPath/probesClosedBinding/bindingChanged`（worktree-view.ts，`samePath` 先统一正斜杠再比——`git worktree list` 给正斜杠、session cwd 带平台分隔符，裸比会把每个 Windows worktree 读成另一个目录）、`repoRootOf`（index.ts）、工具注册在 constructor 的 `ctx.tools.register(defineTool(...))`
